...
const uint8_t CRC8_J1850_TABLE_16[16] = {
    0x00, 0x1D, 0x3A, 0x27,
    0x74, 0x69, 0x4E, 0x53,
    0xE8, 0xF5, 0xD2, 0xCF,
    0x9C, 0x81, 0xA6, 0xBB
...
uint8_t crc8_j1850(const uint8_t* data, uint32_t length)
{
    ...
    while (length != 0u)
    {
        ...
        crc = CRC8_J1850_TABLE_16[(crc >> 4u) ^ ((*data >> 4u) & 0x0Fu)] ^
              (uint8_t)(crc << 4u);

        crc = CRC8_J1850_TABLE_16[(crc >> 4u) ^ (*data & 0x0Fu)] ^
              (uint8_t)(crc << 4u);

#elif CRC8_TABLE_SIZE == 256

        crc = CRC8_J1850_TABLE[crc ^ *data];
